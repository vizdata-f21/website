...
Some guidance for choosing a dataset is below:
...
-   Week 1 of project (week of Mon, Sep 13): Choose a dataset and write up your proposal in `proposal.Rmd`.
    ...
    -   A plan for answering each of the questions including the variables involved, variables to be created (if any), external data to be merged in (if any), and pseudo code
...
-   Write-up and presentation: due Fri, Oct 1 at noon (class time).

## Guidelines

Please use the project repository that has been created for your team for to complete your project.
This means putting all of the content in the R Markdown files provided and knitting them to obtain the output and committing and pushing all source and output files to your repository by the indicated deadlines.
...
Any answers/results given without the corresponding R code that generated the result will be considered absent.
To be clear: if you do calculations by hand instead of using R and then report the results from the calculations, you will not receive credit for those calculations.
...
-   `/data/*`: Your dataset
    ...
    -   `/data/*.*`: in CSV, RDS, or similar format
...
This project consists of three parts, each answering a question you come up with.
Each part should be structured as follows:
...
[^project-1-2]: Your proposal score will also take into consideration how much you've improved your proposal based on peer evaluation feedback.
